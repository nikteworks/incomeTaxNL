...
test('Box 1 mobile card leaves the live net salary result in the first screen', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 800 })
  await page.goto('/?lang=en&calcType=box1')
  await page.locator('#guided-salary').fill('83000')
  const card = page.locator('.box1-form__compact-card')
  const result = page.locator('.guided-headline')
  await expect(card).toBeVisible()
  await expect(result).toBeVisible()
  const cardBounds = await card.boundingBox()
  const resultBounds = await result.boundingBox()
  expect(cardBounds.width).toBeLessThan(390)
  expect(resultBounds.y).toBeGreaterThan(cardBounds.y + cardBounds.height)
  expect(resultBounds.y + resultBounds.height).toBeLessThanOrEqual(800)
  await expect(page.getByLabel(translations.en.box1Form.holidayAllowanceIncluded)).toHaveCount(0)
  await page.setViewportSize({ width: 320, height: 640 })
  const narrowCard = await card.boundingBox()
  const narrowResult = await result.boundingBox()
  expect(narrowCard.x + narrowCard.width).toBeLessThanOrEqual(320)
  expect(narrowResult.y).toBeLessThan(640)
  await page.getByRole('button', { name: translations.en.box1Form.advancedOptions }).click()
  await expect(page.getByRole('dialog').getByLabel(translations.en.box1Form.holidayAllowanceIncluded)).toBeVisible()
})
...
test('Box 1 switches, ruling categories, year and hours reach the library', async ({ page }) => {
  ...
  const advanced = page.getByRole('button', { name: copy.box1Form.advancedOptions })
  await advanced.click()
  await page.getByRole('dialog').getByLabel(copy.box1Form.holidayAllowanceIncluded).uncheck()
  ...
  for (const [label, choice] of [['researchWorker', 'research'], ['youngProfessional', 'young'], ['other', 'normal']]) {
    await advanced.click()
    await page.getByRole('dialog').getByRole('radio', { name: copy.box1Form[label], exact: true }).check()
    await page.keyboard.press('Escape')
  ...
  await dialog.getByRole('switch', { name: copy.guidedRail.pensionAge, exact: true }).check()
  await dialog.getByRole('switch', { name: copy.box1Form.socialSecurity, exact: true }).uncheck()
  await page.keyboard.press('Escape')
