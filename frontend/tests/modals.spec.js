...
for (const language of ['en', 'nl']) {
  for (const width of [390, 768, 1440]) {
    test(`shared dialogs fit and restore focus: ${language}, ${width}px`, async ({ page }) => {
      ...
      await page.getByRole('button', { name: copy.box1Form.advancedOptions, exact: true }).click()
      await page.getByRole('dialog').getByRole('button', { name: copy.box1Form.reset, exact: true }).click()
      await expect(page.getByRole('dialog', { name: copy.box1Form.resetTitle })).toBeVisible()
      await page.keyboard.press('Escape')
      await expect(page.getByRole('dialog', { name: copy.box1Form.advancedOptions })).toBeVisible()
      await page.keyboard.press('Escape')
  ...
  test(`Box 1 advanced modal preserves controls and main ruling: ${language}`, async ({ page }) => {
    ...
    expect(await dialog.evaluate(el => el.scrollWidth <= el.clientWidth)).toBe(true)
    const holidayHelp = dialog.getByRole('button', { name: copy.guidedRail.holidayHelpLabel })
    await dialog.getByRole('switch', { name: copy.box1Form.holidayAllowanceIncluded }).focus()
    await page.keyboard.press('Tab')
    await expect(holidayHelp).toBeFocused()
    await expect(page.getByRole('tooltip')).toContainText(copy.guidedRail.holidayHelp)
    await holidayHelp.click()
    await expect(dialog.getByRole('switch', { name: copy.box1Form.holidayAllowanceIncluded })).toBeChecked()
    await expect(dialog.getByRole('combobox', { name: copy.box1Form.period })).toHaveCount(0)
    await dialog.getByRole('radio', { name: copy.box1Form.researchWorker }).check()
    await expect(page.locator('.box1-form__main-ruling .box1-form__ruling-options')).toHaveCount(0)
    await dialog.getByLabel(copy.box1Form.hoursPerWeek).fill('32')
    await page.keyboard.press('Escape')
    await expect(trigger).toBeFocused()
    await trigger.click()
    await expect(dialog.getByRole('radio', { name: copy.box1Form.researchWorker })).toBeChecked()
